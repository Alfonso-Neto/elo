...
import {
  ...
  MoreHorizontal, RotateCcw, Salad, Search, Sparkles, UserRound, Users,
} from 'lucide-react'
...
function Sidebar() {
  ...
  return <aside className="sidebar"><Brand /><div className="workspace-label"><span>{role === 'trainer' ? 'STUDIO ANDRÉ' : 'MARINA COSTA'}</span><small>{role === 'trainer' ? '28 alunos ativos' : 'Acompanhamento ativo'}</small></div><nav aria-label="Navegação principal">{nav.map(({ page: target, label, icon: Icon }) => <button key={target} className={page === target || (target === 'students' && page === 'student-detail') || (target === 'forms' && page === 'form-builder') ? 'nav-item active' : 'nav-item'} onClick={() => { if (target === 'copilot' || target === 'builder' || target === 'forms') setSelectedStudentId('marina'); if (target !== 'more') navigate(target) }} aria-label={label} title={label} aria-current={page === target ? 'page' : undefined}><Icon size={19} strokeWidth={1.8} /><span>{label}</span>{label === 'Alunos' && openStudents > 0 && <b>{openStudents}</b>}{label === 'Conversas' && unreadMessages > 0 && <b>{unreadMessages}</b>}</button>)}</nav><div className="sidebar-bottom"><RoleSwitch /><button className="reset-button" onClick={resetPrototype}><RotateCcw size={15} /> Reiniciar demonstração</button><div className="sidebar-profile"><span className="avatar">{role === 'trainer' ? 'AL' : 'MC'}</span><div><strong>{role === 'trainer' ? 'André Lima' : 'Marina Costa'}</strong><small>{role === 'trainer' ? 'Treinador · CREF 000000' : 'Aluna · plano ativo'}</small></div><MoreHorizontal size={17} /></div></div></aside>
}
...
export function App() { return <PrototypeProvider><AppContent /></PrototypeProvider> }
