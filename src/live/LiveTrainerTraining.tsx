...
import { useEloApp, type FormSessionDraft } from '../app-state'
...
function useTrainerTarget() {
  ...
  const { selectedStudentId, setSelectedStudentId } = useEloApp()
...
function TargetState({ phase, error, onRetry }: { phase: LoadPhase; error: string; onRetry: () => void }) {
  if (phase === 'loading') return <div className="live-loading"><LoaderCircle className="spin" size={23} /><p>Carregando o contexto do aluno...</p></div>
...
export function LiveWorkoutBuilderScreen() {
  ...
  return <div className="page builder-page live-training-screen enter"><BackButton onClick={() => navigate('copilot')} label="Voltar ao Copiloto" />
    ...
    <button type="button" className="add-block" disabled={publishing} onClick={() => setLibraryOpen(true)}><Plus size={19} /><span><strong>Adicionar exercício</strong><small>Biblioteca de movimentos e parâmetros editáveis</small></span></button>
...
export function LiveTrainerFormsScreen() {
  const { navigate, setFormQuestions, setFormTitle, setFormDraftStudentId } = useEloApp()
  const target = useTrainerTarget()
  ...
  return <div className="page live-training-screen enter"><PageIntro eyebrow="ANAMNESE · DADO COM FINALIDADE" title={<>Pergunte melhor.<br />Prescreva com mais história.</>} copy="Cada envio é imutável, exige vínculo ativo e depende do consentimento vigente do aluno." action={<Button onClick={() => { setFormQuestions([{ id: `q-${Date.now()}`, label: '', type: 'text', required: true }]); setFormTitle('Nova anamnese'); setFormDraftStudentId(target.selectedStudentId); navigate('form-builder') }}><FilePlus2 size={16} /> Criar do zero</Button>} />
    ...
    <SectionTitle index="01" title="Modelos prontos" copy="Pontos de partida editáveis; nenhum modelo é enviado automaticamente." /><div className="template-grid">{formTemplates.map((template,index) => <button type="button" key={template.id} onClick={() => openTemplate(template.id)}><span>{String(index + 1).padStart(2,'0')}</span><FileCheck2 size={21} /><h3>{template.name}</h3><p>{template.niche}</p><footer>{template.questions} perguntas <ArrowRight size={15} /></footer></button>)}</div>
    <section className="section-block"><SectionTitle index="02" title="Histórico do aluno" copy="Respostas só aparecem enquanto a base de acesso e consentimento permitir." />
...
export function LiveFormBuilderScreen() {
  ...
  return <div className="page form-builder-page live-training-screen enter">
    ...
    <div className="question-builder-list">{formQuestions.map((question,index) => <article key={question.id} className="question-card">
      <header>
        ...
        <div className="question-actions">
          <button type="button" onClick={() => move(index,-1)} disabled={sending || index === 0} aria-label="Mover pergunta para cima"><ArrowUp size={15} /></button>
          <button type="button" onClick={() => move(index,1)} disabled={sending || index === formQuestions.length - 1} aria-label="Mover pergunta para baixo"><ArrowDown size={15} /></button>
          <button type="button" className="danger-action" disabled={sending} onClick={() => { if (sending) return; const next = formQuestions.filter((item) => item.id !== question.id); changed(); setFormQuestions(next); rememberDraft(formTitle, next) }} aria-label="Remover pergunta"><Trash2 size={15} /></button>
        </div>
      </header>
      <div className="question-types">{questionTypes.map((type) => <button type="button" className={question.type === type.value ? 'active' : ''} aria-pressed={question.type === type.value} disabled={sending} key={type.value} onClick={() => update(question.id,{ type:type.value })}>{type.label}</button>)}</div>
      {(question.type === 'single' || question.type === 'multi') && <div className="options-editor">
        {(question.options ?? []).map((option,optionIndex) => <label key={`${question.id}-${optionIndex}`}><i /><input value={option} disabled={sending} onChange={(event) => update(question.id,{ options:question.options?.map((item,i) => i === optionIndex ? event.target.value.slice(0,100) : item) })} /><button type="button" disabled={sending || (question.options?.length ?? 0) <= 2} onClick={() => update(question.id,{ options:question.options?.filter((_,i) => i !== optionIndex) })} aria-label="Remover opção"><X size={14} /></button></label>)}
        <button type="button" disabled={sending || (question.options?.length ?? 0) >= 20} onClick={() => update(question.id,{ options:[...(question.options ?? []),`Opção ${(question.options?.length ?? 0) + 1}`] })}>+ adicionar opção</button>
      </div>}
    ...
    <button type="button" className="add-block" disabled={sending || formQuestions.length >= 50} onClick={() => add()}><Plus size={19} /><span><strong>Adicionar pergunta</strong><small>{formQuestions.length >= 50 ? 'Limite seguro de 50 perguntas atingido' : 'Texto, escolha, escala, sim/não ou número'}</small></span></button>
