...
import { createSignalService, type PainReportSummary } from '../signals'
...
export type LiveSignalQueueItem = {
  ...
  studentName: string
  count: number
  latest: PainReportSummary
  critical: boolean
}

export function buildLiveSignalQueue(students: EnrolledStudent[], reports: PainReportSummary[]): LiveSignalQueueItem[] {
...
export function LiveTrainerDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextStudents, reportPage] = await Promise.all([
        ...
        createSignalService().listWorkspaceReports(membership.workspaceId, { limit: 40 }),
      ])
  ...
  return <div className="page enter live-dashboard">
    ...
    <section className="section-block" aria-live="polite" aria-busy={phase === 'loading'}>
      <SectionTitle index="01" title="Precisam de você" copy="Alertas estruturados primeiro; depois intensidade e recência." action={<button className="text-link" onClick={() => void load()} disabled={phase === 'loading'}><RefreshCw className={phase === 'loading' ? 'spin' : ''} size={15} /> Atualizar</button>} />
      {phase === 'loading' && <div className="live-loading"><LoaderCircle className="spin" size={23} /><p>Organizando os sinais deste espaço...</p></div>}
      {phase === 'error' && <div className="empty-state compact"><ShieldCheck size={27} /><h3>O contexto não abriu agora</h3><p>{error}</p><Button variant="secondary" onClick={() => void load()}>Tentar novamente</Button></div>}
      {phase === 'ready' && queue.length === 0 && <div className="empty-state"><HeartPulse size={29} /><h3>Nenhum relato de dor recebido.</h3><p>Quando um aluno vinculado concluir o relato com consentimento, o sinal aparecerá aqui.</p><Button variant="secondary" onClick={() => navigate('students')}><Users size={16} /> Ver alunos vinculados</Button></div>}
      {phase === 'ready' && queue.length > 0 && <div className="attention-list">{queue.map((item, index) => <button className="attention-row" key={item.studentId} onClick={() => openStudent(item.studentId)}>
        <span className={`status-line ${item.critical ? 'danger' : 'warning'}`} /><span className={`signal-avatar ${item.critical ? 'danger' : 'warning'}`}><HeartPulse size={17} /></span><span className="person"><strong>{item.studentName}</strong><small>{item.latest.region} · {item.latest.movement} · intensidade {item.latest.intensity}/10 · {timeLabel(item.latest.createdAt)}</small></span><span className={`tag ${item.critical ? 'danger' : 'warning'}`}>{item.critical ? 'Alerta' : `${item.count} ${item.count === 1 ? 'relato' : 'relatos'}`}</span><ArrowRight size={18} /><span className="row-number">{String(index + 1).padStart(2, '0')}</span>
      </button>)}</div>}
    </section>

    <section className="lower-grid live-lower-grid">
      <article className="surface-card live-operation-card"><SectionTitle index="02" title="Operação real" /><div><span><Users size={18} /><strong>{students.length}</strong><small>alunos ativos</small></span><span><HeartPulse size={18} /><strong>{reports.length}</strong><small>relatos recentes</small></span><span><ShieldCheck size={18} /><strong>{criticalCount}</strong><small>com alerta</small></span></div><Button variant="secondary" onClick={() => navigate('students')}>Gerenciar vínculos <ArrowRight size={15} /></Button></article>
