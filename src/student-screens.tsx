...
import {
  Activity, ArrowRight, CalendarCheck, CalendarDays, Check, CheckCircle2, ChevronRight, Circle,
  Clock3, Dumbbell, FileCheck2, HeartPulse, Info, MessageCircle, Minus, Moon, Plus, Salad,
...
type PainStep = 'intro' | 'location' | 'moment' | 'intensity' | 'detail' | 'done'
...
export function StudentAssistantScreen() {
  ...
  const submitPain = () => { addPainReport({ studentId: 'marina', studentName: 'Marina Costa', location, moment: detail ? `${moment} · ${detail}` : moment, intensity }); setStep('done') }
  const reset = () => { setMode('home'); setStep('intro'); setLocation(''); setMoment(''); setIntensity(0); setDetail('') }
  const backPain = () => {
    if (step === 'location') reset()
    else if (step === 'moment') { setLocation(''); setStep('location') }
    else if (step === 'intensity') { setMoment(''); setStep('moment') }
    else if (step === 'detail') setStep('intensity')
  }
  return <div className="page assistant-page enter"><PageIntro eyebrow="ASSISTENTE · CANAL ESTRUTURADO" title={<>O que seu corpo<br />está tentando dizer?</>} copy="Eu organizo o relato e aviso o André. Não faço diagnóstico e não altero seu treino sozinho." />
    ...
    {mode === 'pain' && <section className="assistant-flow">{step !== 'done' && <BackButton onClick={backPain} label="Voltar uma etapa" />}<div className="assistant-thread"><div className="assistant-message"><Sparkles size={17} /><p>{step === 'done' ? 'Seu relato foi estruturado e já apareceu para o André.' : 'Vou fazer algumas perguntas objetivas. Se a dor for súbita, intensa, vier com inchaço importante ou perda de movimento, interrompa o treino e procure avaliação profissional.'}</p></div>{location && <div className="assistant-answer">{location}</div>}{moment && <div className="assistant-answer">{moment}</div>}{step === 'detail' && <div className="assistant-answer">Intensidade {intensity}/10</div>}</div>
      {step === 'location' && <FlowQuestion title="Onde você sentiu?" copy="Escolha a região mais próxima.">{['Joelho direito','Joelho esquerdo','Lombar','Ombro','Quadril','Outra região'].map((item) => <button key={item} onClick={() => { setLocation(item); setStep('moment') }}>{item}</button>)}</FlowQuestion>}
      ...
      {step === 'intensity' && <div className="flow-question"><Eyebrow>PASSO 3 DE 4</Eyebrow><h3>Qual foi a intensidade?</h3><p>0 é sem dor; 10 é a pior dor imaginável.</p><div className="pain-scale">{Array.from({ length: 11 }, (_, value) => <button className={intensity === value ? 'active' : ''} key={value} onClick={() => { setIntensity(value); setStep(value >= 8 ? 'detail' : 'detail') }}>{value}</button>)}</div></div>}
      {step === 'detail' && <div className="flow-question"><Eyebrow>ÚLTIMO PASSO · OPCIONAL</Eyebrow><h3>Algum detalhe importante?</h3><p>{intensity >= 8 ? 'Como a intensidade é alta, pare o treino agora. Se houver trauma, inchaço importante ou perda de movimento, procure atendimento.' : 'Uma frase já basta. O relato pode ser enviado sem esse campo.'}</p><textarea value={detail} onChange={(event) => setDetail(event.target.value.slice(0, 300))} placeholder="Ex.: começou na terceira série e melhorou quando parei..." /><Button className="wide" onClick={submitPain}><Send size={16} /> Enviar ao André</Button></div>}
      {step === 'done' && <SuccessState title="O André já recebeu" copy={`${location} · ${moment} · intensidade ${intensity}/10. O dado ficou anexado ao seu histórico.`} action={<div className="success-actions"><Button onClick={() => navigate('today')}>Voltar para hoje</Button><Button variant="secondary" onClick={reset}>Novo relato</Button></div>} />}
    </section>}
