...
function DemoStudentWorkoutScreen() {
  const { navigate, openExercisePainReport, studentWorkout: workout, studentWorkoutName: workoutName, completedExercises, setCompletedExercises, submitWorkoutFeedback, notify } = usePrototype()
...
export function StudentAssistantScreen() {
  const { navigate, addPainReport, assistantEntry, clearAssistantEntry, studentWorkout: demoWorkout, setSessions, notify } = usePrototype()
  ...
  return <div className="page assistant-page enter"><PageIntro eyebrow="ASSISTENTE · CANAL ESTRUTURADO" title={<>O que seu corpo<br />está tentando dizer?</>} copy={`Eu organizo o relato e aviso ${trainerFirstName}. Não faço diagnóstico e não altero seu treino sozinho.`} />
    ...
    {mode === 'pain' && <section className="assistant-flow">{step !== 'done' && !submitting && !submitError && <BackButton onClick={backPain} label="Voltar uma etapa" />}{movementSource === 'workout' && step !== 'done' && <div className="assistant-entry-context" role="status"><span><Dumbbell size={17} /></span><div><Eyebrow>CONTEXTO DO TREINO</Eyebrow><strong>{movement}</strong><small>O exercício foi trazido do treino aberto. Você ainda confirma todo o relato.</small></div><button type="button" onClick={() => { setMovement(''); setMovementSource(null); setStep(location ? 'movement' : 'location') }}>Trocar movimento</button></div>}<div className="assistant-thread"><div className="assistant-message"><Sparkles size={17} /><p>{step === 'done' ? `Seu relato foi estruturado e já apareceu para ${trainerFirstName}.` : 'Vou organizar o que aconteceu e checar sinais de alerta. Eu não faço diagnóstico e não altero seu treino.'}</p></div>{location && <div className="assistant-answer">{location}</div>}{location && movement && <div className="assistant-answer">{movement}</div>}{moment && <div className="assistant-answer">{moment}</div>}{['red-flags','detail','review'].includes(step) && <div className="assistant-answer">Intensidade {intensity}/10</div>}</div>
      {step === 'location' && <FlowQuestion title="Onde você sentiu?" copy="Escolha a região mais próxima.">{['Joelho direito','Joelho esquerdo','Lombar','Ombro direito','Ombro esquerdo','Quadril','Outra região'].map((item) => <button key={item} onClick={() => { setLocation(item); setStep(movementSource === 'workout' && movement ? 'moment' : 'movement') }}>{item}</button>)}</FlowQuestion>}
      {step === 'movement' && <FlowQuestion title="Em qual movimento?" copy="Escolha o exercício ou a situação mais próxima.">{movementOptions.map((item) => <button key={item} onClick={() => { setMovement(item); setStep('moment') }}>{item}</button>)}</FlowQuestion>}
...
function DemoStudentFormScreen() {
  ...
  const submit = () => {
    ...
    setErrors(missing)
    if (!missing.length) { submitForm(answers); return }
    window.requestAnimationFrame(() => {
      if (missing[0] === 'consent') { document.getElementById('student-form-consent')?.focus(); return }
      const fieldset = Array.from(document.querySelectorAll<HTMLElement>('[data-question-id]')).find((item) => item.dataset.questionId === missing[0])
      fieldset?.querySelector<HTMLElement>('input, textarea, button')?.focus()
    })
  }
  ...
  return <div className="page form-fill-page enter"><BackButton onClick={() => navigate('today')} /><PageIntro eyebrow={`${publishedFormTitle.toUpperCase()} · ANDRÉ LIMA`} title="Antes do treino, sua história." copy="Reserve cerca de três minutos. Você pode revisar as respostas antes de enviar." />
    <section className={errors.includes('consent') ? 'consent-card error' : 'consent-card'}><FileCheck2 size={22} /><div><h3>Consentimento para dados de saúde</h3><p>Autorizo o uso destas respostas exclusivamente para avaliação, planejamento e acompanhamento do meu treino. Posso solicitar acesso, correção ou exclusão.</p><label className="switch-label"><input id="student-form-consent" type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} aria-invalid={errors.includes('consent')} aria-describedby={errors.includes('consent') ? 'student-form-consent-error' : undefined} /><i /><span>Li e concordo com o uso descrito acima.</span></label>{errors.includes('consent') && <small id="student-form-consent-error" role="alert">Você precisa registrar o consentimento antes de enviar.</small>}</div></section>
    <div className="fill-form">{formQuestions.map((question, index) => {
      const invalid = errors.includes(question.id)
      const errorId = `student-form-${question.id}-error`
      return <fieldset className={invalid ? 'error' : ''} data-question-id={question.id} key={question.id}><legend><Eyebrow>{String(index + 1).padStart(2,'0')} · {question.required ? 'OBRIGATÓRIA' : 'OPCIONAL'}</Eyebrow><span>{question.label}</span></legend>{question.type === 'long' ? <textarea value={(answers[question.id] as string) ?? ''} onChange={(event) => update(question, event.target.value)} placeholder={question.id === 'q3' ? 'Escreva com suas palavras...' : 'Conte o que considerar importante...'} aria-label={question.label} aria-required={question.required} aria-invalid={invalid} aria-describedby={invalid ? errorId : undefined} /> : question.type === 'scale' ? <div className="answer-scale" role="group" aria-label={question.label} aria-invalid={invalid} aria-describedby={invalid ? errorId : undefined}>{Array.from({ length: 11 }, (_, value) => { const active = answers[question.id] === String(value); return <button type="button" className={active ? 'active' : ''} aria-pressed={active} key={value} onClick={() => update(question, String(value))}>{value}</button> })}</div> : ['single','multi','yesno'].includes(question.type) ? <div className="answer-options" role="group" aria-label={question.label} aria-invalid={invalid} aria-describedby={invalid ? errorId : undefined}>{(question.type === 'yesno' ? ['Sim','Não'] : question.options ?? []).map((option) => { const active = Array.isArray(answers[question.id]) ? (answers[question.id] as string[]).includes(option) : answers[question.id] === option; return <button type="button" className={active ? 'active' : ''} aria-pressed={active} key={option} onClick={() => update(question, option, question.type === 'multi')}><i>{active && <Check size={13} />}</i>{option}</button> })}</div> : <input type={question.type === 'number' ? 'number' : 'text'} value={(answers[question.id] as string) ?? ''} onChange={(event) => update(question, event.target.value)} placeholder="Sua resposta" aria-label={question.label} aria-required={question.required} aria-invalid={invalid} aria-describedby={invalid ? errorId : undefined} />}{invalid && <small id={errorId} role="alert">Responda esta pergunta para continuar.</small>}</fieldset>
    })}</div>
