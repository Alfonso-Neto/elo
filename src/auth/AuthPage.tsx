...
function AuthLayout({ route, navigate, children }: { route: AuthRoute; navigate: (route: AuthRoute) => void; children: ReactNode }) {
  const { configured, configurationIssue } = useAuth()
  return <div className="auth-shell">
    <aside className="auth-story" aria-label="Elo, cuidado com contexto">
      ...
      <div className="auth-story-copy">
        ...
        <p>O Elo aproxima professor e aluno para transformar sinais do dia a dia em decisões melhores.</p>
      </div>
    ...
    <main className="auth-main" id="main-content">
      ...
      <nav className="auth-nav" aria-label="Acesso">
        <button type="button" className={route === 'entrar' ? 'active' : ''} onClick={() => navigate('entrar')}>Entrar</button>
        <button type="button" className={route === 'cadastro' ? 'active' : ''} onClick={() => navigate('cadastro')}>Criar conta</button>
      </nav>
      <section className="auth-panel">{children}</section>
      {!configured && <div className="auth-environment" role="status">
        <ShieldCheck size={17} />
        <p><strong>Ambiente sem conexão de autenticação.</strong>{configurationIssue ?? ' Configure as variáveis públicas do Supabase para habilitar as contas.'}</p>
      </div>}
  ...
  return <>
    ...
    <form className="auth-form" onSubmit={submit} noValidate>
      ...
      <AuthField label="E-mail" fieldId="login-email" error={errors.email}>
        <span className="auth-input-wrap"><AtSign size={17} aria-hidden="true" /><input id="login-email" type="email" maxLength={320} value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" inputMode="email" placeholder="voce@exemplo.com" aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? 'login-email-error' : undefined} /></span>
      </AuthField>
    ...
    <p className="auth-switch-copy">Ainda não faz parte? <button type="button" onClick={() => navigate('cadastro')}>Crie sua conta</button></p>
  </>
}
...
function RoleChoice({ onChoose }: { onChoose: (role: Role) => void }) {
  return <>
    ...
    <div className="auth-role-grid" role="group" aria-label="Escolha seu perfil">
      <button type="button" onClick={() => onChoose('trainer')}>
      ...
      <button type="button" onClick={() => onChoose('student')}>
...
function SignUp({ navigate, onEmail }: { navigate: (route: AuthRoute) => void; onEmail: (email: string) => void }) {
  ...
  return <>
    <button type="button" className="auth-back" onClick={() => setStep('role')}><ArrowLeft size={16} /> Trocar perfil</button>
    ...
    <form className="auth-form" onSubmit={submit} noValidate>
      ...
      <div className="auth-two-columns">
        <AuthField label="Nome completo" fieldId="signup-name" error={errors.displayName}>
          <span className="auth-input-wrap"><UserRound size={17} aria-hidden="true" /><input id="signup-name" maxLength={80} value={values.displayName} onChange={(event) => update('displayName', event.target.value)} autoComplete="name" placeholder="Como devemos chamar você?" aria-invalid={Boolean(errors.displayName)} aria-describedby={errors.displayName ? 'signup-name-error' : undefined} /></span>
        </AuthField>
        <AuthField label="E-mail" fieldId="signup-email" error={errors.email}>
          <span className="auth-input-wrap"><AtSign size={17} aria-hidden="true" /><input id="signup-email" type="email" maxLength={320} value={values.email} onChange={(event) => update('email', event.target.value)} autoComplete="email" inputMode="email" placeholder="voce@exemplo.com" aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? 'signup-email-error' : undefined} /></span>
        </AuthField>
      </div>
      ...
      {values.role === 'trainer' && <div className="trainer-fields" aria-label="Dados profissionais">
        ...
        <div className="auth-professional-grid">
          ...
          <AuthField label="Estúdio ou marca" fieldId="signup-studio" hint="Opcional" error={errors.studioName}>
            <input id="signup-studio" className="auth-plain-input" value={values.studioName} onChange={(event) => update('studioName', event.target.value)} autoComplete="organization" placeholder="Nome do seu espaço" maxLength={80} aria-invalid={Boolean(errors.studioName)} aria-describedby={errors.studioName ? 'signup-studio-error' : 'signup-studio-hint'} />
          </AuthField>
        </div>
      </div>}
    ...
    <p className="auth-switch-copy">Já tem uma conta? <button type="button" onClick={() => navigate('entrar')}>Entre por aqui</button></p>
  </>
}
...
function ConfirmEmail({ email, navigate }: { email: string; navigate: (route: AuthRoute) => void }) {
  ...
  const resend = async () => {
    if (!email || !configured || resendInFlight.current) return
  ...
  return <div className="auth-state">
    ...
    <button type="button" className="auth-submit" onClick={() => navigate('entrar')}>Voltar para entrar <ArrowRight size={18} /></button>
    {email && <button type="button" className="auth-text-button state-link" onClick={resend} disabled={busy || !configured}>{busy ? 'Solicitando…' : 'Reenviar confirmação'}</button>}
  </div>
}
...
function ForgotPassword({ navigate }: { navigate: (route: AuthRoute) => void }) {
  ...
  if (sent) return <div className="auth-state"><span className="auth-state-icon"><MailCheck size={28} /></span><span className="auth-kicker">SOLICITAÇÃO RECEBIDA</span><h2>Confira seu e-mail.</h2><p>Se houver uma conta elegível para esse endereço, enviaremos as instruções de redefinição.</p><button type="button" className="auth-submit" onClick={() => navigate('entrar')}>Voltar para entrar <ArrowRight size={18} /></button></div>
  return <>
    <button type="button" className="auth-back" onClick={() => navigate('entrar')}><ArrowLeft size={16} /> Voltar</button>
