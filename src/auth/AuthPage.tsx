...
function AuthField({ label, error, errorId, hint, children }: { label: string; error?: string; errorId?: string; hint?: string; children: ReactNode }) {
  return <label className={error ? 'auth-field has-error' : 'auth-field'}>
    <span>{label}</span>
    {children}
    {error ? <small id={errorId} className="auth-field-error">{error}</small> : hint ? <small>{hint}</small> : null}
  </label>
}
...
function PasswordInput({ id, value, onChange, autoComplete, placeholder = 'Sua senha', error, label = 'Senha', hint }: {
  ...
  return <AuthField label={label} error={error} errorId={`${id}-error`} hint={hint}>
...
function AuthLayout({ route, navigate, children }: { route: AuthRoute; navigate: (route: AuthRoute) => void; children: ReactNode }) {
  ...
  return <div className="auth-shell">
    <a className="skip-link" href="#auth-main">Ir para o formulário</a>
  ...
  return <>
    ...
    <form className="auth-form" onSubmit={submit} noValidate>
      ...
      <AuthField label="E-mail" error={errors.email}>
        <span className="auth-input-wrap"><AtSign size={17} aria-hidden="true" /><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" inputMode="email" placeholder="voce@exemplo.com" aria-invalid={Boolean(errors.email)} /></span>
      </AuthField>
  ...
  return <>
    ...
    <form className="auth-form" onSubmit={submit} noValidate>
      ...
      <div className="auth-two-columns">
        <AuthField label="Nome completo" error={errors.displayName}>
          <span className="auth-input-wrap"><UserRound size={17} aria-hidden="true" /><input value={values.displayName} onChange={(event) => update('displayName', event.target.value)} autoComplete="name" placeholder="Como devemos chamar você?" aria-invalid={Boolean(errors.displayName)} /></span>
        </AuthField>
        <AuthField label="E-mail" error={errors.email}>
          <span className="auth-input-wrap"><AtSign size={17} aria-hidden="true" /><input type="email" value={values.email} onChange={(event) => update('email', event.target.value)} autoComplete="email" inputMode="email" placeholder="voce@exemplo.com" aria-invalid={Boolean(errors.email)} /></span>
        </AuthField>
      </div>
      ...
      {values.role === 'trainer' && <div className="trainer-fields" aria-label="Dados profissionais">
        ...
        <div className="auth-professional-grid">
          <AuthField label="Número do CREF" error={errors.crefNumber}>
            <span className="auth-input-wrap"><BadgeCheck size={17} aria-hidden="true" /><input value={values.crefNumber} onChange={(event) => update('crefNumber', event.target.value.toUpperCase())} autoComplete="off" placeholder="000000-G" aria-invalid={Boolean(errors.crefNumber)} /></span>
          </AuthField>
          <AuthField label="UF do CREF" error={errors.crefState}>
            <select value={values.crefState} onChange={(event) => update('crefState', event.target.value)} aria-invalid={Boolean(errors.crefState)}><option value="">UF</option>{states.map((state) => <option key={state}>{state}</option>)}</select>
          </AuthField>
          <AuthField label="Estúdio ou marca" hint="Opcional">
            <input className="auth-plain-input" value={values.studioName} onChange={(event) => update('studioName', event.target.value)} autoComplete="organization" placeholder="Nome do seu espaço" maxLength={80} />
          </AuthField>
        </div>
      </div>}
      ...
      <label className={errors.acceptedTerms ? 'auth-check has-error' : 'auth-check'}>
        <input type="checkbox" checked={values.acceptedTerms} onChange={(event) => update('acceptedTerms', event.target.checked)} />
...
function ForgotPassword({ navigate }: { navigate: (route: AuthRoute) => void }) {
  ...
  return <>
    ...
    <form className="auth-form" onSubmit={submit} noValidate>
      ...
      <AuthField label="E-mail" error={error && !error.includes('autenticação') ? error : undefined}>
        <span className="auth-input-wrap"><AtSign size={17} aria-hidden="true" /><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" inputMode="email" placeholder="voce@exemplo.com" aria-invalid={Boolean(error)} /></span>
      </AuthField>
